...
//ToDo:SudokuMovementMessage
//ToDo:SudokuWinnerMessage
...
public class PartidaView extends View {
    ...
    public PartidaView(Context context) {
        ...
        bordeGrueso.setColor(Color.RED);
        ...
        mPropia = (int) numerosFijosAjenos.measureText("M");
        ...
        topAjeno = 0.6f * ScreenParameters.screenHeight;
        ...
        leftTeclado = 0.5f * ScreenParameters.screenWidth;
        rigthTeclado = 0.95f * ScreenParameters.screenWidth;
        board = new int[9][9];
    }
        @Override
        ...
        if(!partidaFinalizada){
            ...
            escribirNumeroFijosPropios(canvas);
            escribirNumeroFijosAjenos(canvas);
        ...
        if(event.getAction()== MotionEvent.ACTION_DOWN){
            ...
            if(x>=leftPropio && x<=rightPropio&&y>=topPropio&&y<=bottomPropio){
                ...
                if(this.colCeldaACambiar!=-1&&!tableroPropio[colCeldaACambiar][rowCeldaACambiar].isProtegida()) {
                    ...
                    SudokuMovementMessage movement = new SudokuMovementMessage(rowCeldaACambiar, colCeldaACambiar, numeroAPoner, Store.get().getUser().getIdUser(), Store.get().getIdMatch());
    ...
    private void escribirNumeroFijosPropios(Canvas canvas){
        float row;
        float col;
        for(int i =0;i<9;i++){
            col=leftPropio+altoPropio*(i+1)-(anchoPropio/2);
            for(int j=0;j<9;j++){
    ...
    private void escribirNumeroFijosAjenos(Canvas canvas){
        ...
        for(int i=0;i<9;i++){
            ...
            for(int j=0;j<9;j++){
                ...
                else canvas.drawText(col,row,20,numerosFijosAjenos);
            }
        }
    }
 ...
 private void setBoard(String board){
     ...
     for (int col=0;col<9;col++){
         for(int row=0;row<9;row++){
             ...
             this.board[row][col]=new Casilla(n);
             if(n!=0)
                 this.tableroPropio[row][col].protejer();
         }
     }
 }
public void setCasilla(int row,int col,int valor){
    ...
    public void showVictory(SudokuWinnerMessage swm){
        ...
        mensajc="Ganaste En" +swm.getTime()+"segundos";
        invalidate();
    }
}
